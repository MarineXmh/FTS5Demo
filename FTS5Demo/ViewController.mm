...
extern "C"{
#import "sqlite_fts_pinyin_util.h"
}
...
- (void)testCPPFunctions {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        const char *pinyin = "xu meng hua ";
        string pinyinString = transferPinyinToSearchFormat(pinyin);
        map<string,string> dataMap;
        dataMap.insert(make_pair("data_id", dataIdStr));
        dataMap.insert(make_pair("type", "666"));
        dataMap.insert(make_pair("body", pinyinString));
    ...
    NSArray *result = [SqliteFTSUtil selectWithTable:@"simple_fts" Column:@"simple_fts" Match:@"_xu"];
