...
- (void)prepareTestData {
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        NSString *idString = [NSString stringWithFormat:@"%06d", i];
        NSString *message = [text substringWithRange:range];
        NSDictionary *data = @{@"id":idString, @"message":message};
        [dataArray addObject:data];
    }
    [SqliteFTSUtil insertArrayToMessage:dataArray];
...
- (void)testMatch {
    ...
    NSDate *startTime = [NSDate date];
    NSLog(@"match started");
    NSArray *result = [SqliteFTSUtil selectWithTable:@"message_fts" Column:@"message_fts" Match:@"蛤三篇"];
    NSDate *finishTime = [NSDate date];
    NSTimeInterval time = [finishTime timeIntervalSinceDate:startTime];
...
- (void)testSplite {
    ...
    CFStringRef textRef = CFStringCreateWithCString(kCFAllocatorDefault, text, kCFStringEncodingUTF8);
    CFStringRef characterRef = CFStringCreateWithSubstring(kCFAllocatorDefault, textRef, CFRangeMake(0, 1));
    CFRelease(textRef);
    CFRelease(characterRef);
}

- (void)testFTS {
    insert_feed("蛤蛤asd123", "qwer", "zxc", "vbn");
    insert_feed("asd123", "qwer", "zxc", "vbn");
    NSArray *match0 = [SqliteFTSUtil selectWithTable:@"feed_fts" Column:@"feed_fts" Match:@"asd"];
    NSLog(@"%@", match0);
    update_feed("feed_id='蛤蛤asd888'", "feed_id='蛤蛤asd123'");
    update_feed("feed_id='asd888'", "feed_id='asd123'");
    NSArray *match1 = [SqliteFTSUtil selectWithTable:@"feed_fts" Column:@"feed_fts" Match:@"asd"];
    NSLog(@"%@", match1);
    delete_feed("feed_id='蛤蛤asd888'");
    delete_feed("feed_id='asd888'");
    NSArray *match2 = [SqliteFTSUtil selectWithTable:@"feed_fts" Column:@"feed_fts" Match:@"asd"];
    NSLog(@"%@", match2);
    
    insert_message("00000001", "asdfghjkl");
    NSArray *match3 = [SqliteFTSUtil selectWithTable:@"message_fts" Column:@"message_fts" Match:@"as"];
    NSLog(@"%@", match3);
    update_message("message='qwerty'", "id='00000001'");
    NSArray *match4 = [SqliteFTSUtil selectWithTable:@"message_fts" Column:@"message_fts" Match:@"q"];
    NSLog(@"%@", match4);
    delete_message("id='00000001'");
    NSArray *match5 = [SqliteFTSUtil selectWithTable:@"message_fts" Column:@"message_fts" Match:@"qwer"];
    NSLog(@"%@", match5);
}
